...
import { navigationAutomation, navigationHistory, navigationQuests, navigationBattle, navigationTown, rejectNavigationMutation } from './navigationFixtures';
...
const hub = new CharacterManagementHubModule({
  loadStoredDetail: async (id) => {
    ...
    return makeHofCharacterDetail(id, {
      ...
      equipment: Array.from({ length: 30 }, (_, index) => ({ slot: `slot-${index}`, part: `부위 ${index + 1}`, name: '검증용 장비', iconUrl: '', description: '긴 장비 목록 스크롤 검증', checked: true })),
      actionPatterns: Array.from({ length: 24 }, (_, index) => ({ index, judge: 'always', judgeText: '항상', quantity: '0', quantityText: '', skill: 'attack', skillText: '공격' })),
      learnedSkills: Array.from({ length: 50 }, (_, index) => ({ value: String(index), name: `검증 스킬 ${index + 1}`, iconUrl: '', category: '검증', description: '스크롤 검증용 스킬' })),
      patternSlots: Array.from({ length: 40 }, (_, index) => ({ slot: String(index), label: `검증 저장 패턴 ${index + 1}`, canLoad: true })),
      patternOptions: [
        ...Array.from({ length: 40 }, (_, index) => ({ type: "CLASS" as const, value: `class-${index}`, label: `검증 직업 ${index + 1}`, category: null })),
  ...
  loadCurrentOperation: async (id) => id == null || id === 5 ? recoveryJob : null,
  previewRecovery: async () => ({
    ...
    hofCharacterId: '5', name: '검증 05', patterns: Array.from({ length: 40 }, (_, index) => ({ index, judge: 'always', judgeText: '항상', quantity: '0', quantityText: '', skill: 'attack', skillText: '공격' })), equipment: [],
...
const unavailable = () => rejectNavigationMutation();
const automationController = new UnifiedAutomationController({
  fetch: async () => navigationAutomation, fetchHistory: navigationHistory,
  fetchConvergence: async () => ({ items: [], battleGate: null, localResults: [] }), create: unavailable, delete: unavailable, reorder: unavailable,
  updateQuest: unavailable, updateBattle: unavailable, updateAdventure: unavailable,
  fetchQuests: async () => navigationQuests, changeState: unavailable,
});
const battle = navigationBattle;
...
function NavigationApp() {
  ...
  return <GestureHandlerRootView style={{ flex: 1 }}><SafeAreaProvider>
    <MainScreen session={{ loggedIn: true }} status={null} notice={null} battle={battle}
      townApi={navigationTown} characterHub={characterHub} automationController={automationController} partyPresetCatalog={partyPresetCatalog}
