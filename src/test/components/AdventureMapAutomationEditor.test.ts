...
describe('AdventureMapAutomationEditor', () => {
  ...
  it('protects a name-only edit through continue editing, discard, and successful save', async () => {
    alertArguments = null;
    let backs = 0;
    const saves: Array<UpdateAdventureMapAutomationRequest & { displayName?: string | null }> = [];
    const renderer = await renderEditor({
      onBack: () => { backs += 1; },
      onSave: async (request) => { saves.push(request); return true; },
    });
    const name = () => renderer.root.findByProps({ accessibilityLabel: '모험맵 묶음 이름' });
    const back = () => renderer.root.findByProps({ accessibilityLabel: '모험맵 자동화 뒤로' }).props.onPress();
  ...
  it('fences retained name, save, back and discard callbacks and permits retry after failure', async () => {
    ...
    const retainedSave = renderer.root.findByProps({ accessibilityLabel: '모험맵 자동화 저장' }).props.onPress as () => Promise<void>;
  ...
  it('keeps a different server name dirty after save and isolates a late completion from a new editor generation', async () => {
    ...
    await act(async () => { renderer.update(React.createElement(AdventureMapAutomationEditor, {
      ...base, entry: { ...base.entry, displayName: '별도 서버 이름' },
    })); });
    assert.equal(renderer.root.findByProps({ accessibilityLabel: '모험맵 묶음 이름' }).props.value, '별도 서버 이름');
    ...
    await act(async () => { old = create(React.createElement(AdventureMapAutomationEditor, { ...base, onSave: async () => late.promise })); });
    let oldSave!: Promise<void>;
    await act(async () => { oldSave = old.root.findByProps({ accessibilityLabel: '모험맵 자동화 저장' }).props.onPress(); });
    await act(async () => { old.unmount(); });
    activeRenderers.delete(old);
    let backs = 0;
    const fresh = await renderEditor({ onBack: () => { backs += 1; } });
    await act(async () => { fresh.root.findByProps({ accessibilityLabel: '모험맵 묶음 이름' }).props.onChangeText('새 로그인 편집'); });
    await act(async () => { late.resolve(true); await oldSave; });
    alertArguments = null;
    await act(async () => { fresh.root.findByProps({ accessibilityLabel: '모험맵 자동화 뒤로' }).props.onPress(); });
    assert.equal(fresh.root.findByProps({ accessibilityLabel: '모험맵 묶음 이름' }).props.value, '새 로그인 편집');
    assert.equal(backs, 0);
    assert.equal(alertArguments?.[0], '변경 사항을 버릴까요?');
  });
