...
function IdentifyPanel({ api, resolveCaptcha }: Omit<Props, 'mode'>) {
  ...
  return <PanelList rows={data.cards} selectedIds={selected} selectionMode="single" onSelectionChange={setSelected} header={<Text style={styles.muted}>감정할 카드 1장을 선택하세요.</Text>} footer={<View style={styles.section}><ActionButton label="선택 카드 감정" disabled={!card || town.status === 'submitting'} onPress={() => setConfirming(true)} />{data.result ?? town.result ? <TownActionResult result={(data.result ?? town.result)!} /> : null}{town.error ? <Text accessibilityRole="alert" style={styles.error}>{town.error}</Text> : null}</View>} confirm={<TownConfirmSheet visible={confirming} title="카드 감정 확인" message="선택한 카드 1장을 감정합니다." confirmLabel="감정" destructive submitting={town.status === 'submitting'} details={[{ label: '카드', value: card?.label ?? '미선택' }, { label: '비용', value: money(card?.cost) }]} onCancel={() => setConfirming(false)} onConfirm={() => card && void town.submit({ candidateId: card.id }).catch(() => setConfirming(false))} />} />;
}

function PairPanel({ api, mode, resolveCaptcha }: Props & { mode: 'upgrade' | 'change' }) {
  const [selected, setSelected] = useState<string[]>([]); const [quantityText, setQuantityText] = useState('1'); const [confirming, setConfirming] = useState(false); const [response, setResponse] = useState<CardPairResponse | null>(null); const [options, setOptions] = useState<CardPairResponse | null>(null); const [optionsLoading, setOptionsLoading] = useState(false); const [optionsError, setOptionsError] = useState<string | null>(null); const optionsSequence = useRef(0); const mounted = useRef(true);
  ...
  return <PanelList rows={rows} selectedIds={selected} selectionMode="grouped-single" selectionGroup={(it) => it.id.startsWith('base:') ? 'base' : 'material'} onSelectionChange={changeSelection}
    ...
    footer={<View style={styles.section}>{optionsLoading ? <Text accessibilityLiveRegion="polite" style={styles.muted}>추가 카드 후보를 불러오는 중...</Text> : null}{optionsError ? <><Text accessibilityRole="alert" style={styles.error}>{optionsError}</Text>{baseCard ? <ActionButton label="추가 카드 다시 불러오기" disabled={optionsLoading} onPress={() => loadOptions(baseCard.id)} /> : null}</> : null}{baseCard && !optionsLoading && options && options.materialCards.length === 0 ? <Text style={styles.muted}>추가 카드 후보가 없습니다.</Text> : null}<QuantityInput label={`${title} 수량`} value={quantityText} onChange={setQuantityText} min={data.minQuantity} max={selectedMaxQuantity} valid={validQuantity} />{sameCard ? <Text accessibilityRole="alert" style={styles.error}>베이스 카드와 재료 카드는 서로 달라야 합니다.</Text> : null}<ActionButton label={title} disabled={!baseCard || !materialCard || sameCard || !validQuantity || optionsLoading || town.status === 'submitting'} onPress={() => setConfirming(true)} />{data.result ?? town.result ? <TownActionResult result={(data.result ?? town.result)!} /> : null}{town.error ? <Text accessibilityRole="alert" style={styles.error}>{town.error}</Text> : null}</View>}
    confirm={<TownConfirmSheet visible={confirming} title={`${title} 확인`} message={`${title}는 카드와 Blank Card를 소모할 수 있습니다.`} confirmLabel="Create" destructive submitting={town.status === 'submitting'} details={[{ label: '베이스', value: baseCard?.label ?? '미선택' }, { label: mode === 'upgrade' ? '추가 카드' : '변화 재료', value: materialCard?.label ?? '미선택' }, { label: '수량', value: `${quantity}회` }, { label: '표시 비용', value: money(materialCard?.cost ?? baseCard?.cost) }]} onCancel={() => setConfirming(false)} onConfirm={() => baseCard && materialCard && validQuantity && void town.submit({ baseCandidateId: baseCard.id, materialCandidateId: materialCard.id, quantity }).catch(() => setConfirming(false))} />} />;
}
...
function SellPanel({ api, resolveCaptcha }: Omit<Props, 'mode'>) {
  ...
  return <PanelList rows={data.cards} selectedIds={selected} selectionMode="multiple" onSelectionChange={(ids) => { setSelected(ids); setQuantities((current) => ({ ...current, ...Object.fromEntries(ids.map((id) => [id, current[id] ?? '1'])) })); }}
    ...
    footer={<View style={styles.section}>{lines.map(({ card, valid }) => <QuantityInput key={card.id} label={`${card.label} 판매 수량`} value={quantities[card.id] ?? '1'} onChange={(value) => setQuantities((current) => ({ ...current, [card.id]: value }))} min={1} max={card.maxQuantity ?? card.owned ?? 0} valid={valid} />)}<Text style={styles.total}>예상 Blank Card +{expected.toLocaleString()}장</Text>{lines.some(({ card }) => card.blankCardValue == null) ? <Text accessibilityRole="alert" style={styles.error}>일부 카드의 교환량을 확인하지 못해 예상 합계가 실제와 다를 수 있습니다.</Text> : null}{data.blankCardsOwned != null ? <Text style={styles.muted}>현재 보유 {data.blankCardsOwned.toLocaleString()}장</Text> : null}<ActionButton label="선택 카드 판매" disabled={!lines.length || lines.some((it) => !it.valid) || town.status === 'submitting'} onPress={() => setConfirming(true)} />{data.result ?? town.result ? <TownActionResult result={(data.result ?? town.result)!} /> : null}{town.error ? <Text accessibilityRole="alert" style={styles.error}>{town.error}</Text> : null}</View>}
    confirm={<TownConfirmSheet visible={confirming} title="카드 판매 확인" message="선택한 카드는 Blank Card로 교환되며 되돌릴 수 없습니다." confirmLabel="판매" destructive submitting={town.status === 'submitting'} details={[...lines.map(({ card, quantity }) => ({ label: card.label, value: card.blankCardValue == null ? `${quantity}장 → 교환량 확인 필요` : `${quantity}장 → Blank Card ${(card.blankCardValue * quantity).toLocaleString()}장`, warning: card.blankCardValue == null })), { label: '예상 합계', value: `${expected.toLocaleString()}장` }]} onCancel={() => setConfirming(false)} onConfirm={() => void town.submit({ cards: lines.map(({ card, quantity }) => ({ candidateId: card.id, quantity })) }).catch(() => setConfirming(false))} />} />;
}
...
function SoulEchoPanel({ api, resolveCaptcha }: Omit<Props, 'mode'>) {
  ...
  return <PanelList rows={rows} selectedIds={[...(categoryId ? [`category:${categoryId}`] : []), ...selected]} selectionMode="grouped-single" selectionGroup={(it) => it.id.startsWith('category:') ? 'category' : 'recipe'} onSelectionChange={(ids) => setSelected(ids.filter((id) => id.startsWith('recipe:')))}
    ...
    footer={<View style={styles.section}>{!categoryId ? <Text accessibilityRole="alert" style={styles.error}>현재 품목 분류를 확인하지 못했습니다.</Text> : null}<ActionButton label="소울 에코 융합" disabled={!categoryId || !recipe || town.status === 'submitting'} onPress={() => setConfirming(true)} />{data.result ?? town.result ? <TownActionResult result={(data.result ?? town.result)!} /> : null}{town.error ? <Text accessibilityRole="alert" style={styles.error}>{town.error}</Text> : null}</View>}
    confirm={<TownConfirmSheet visible={confirming} title="소울 에코 융합 확인" message="실패하면 소울 에코가 소비될 수 있습니다." confirmLabel="융합" destructive submitting={town.status === 'submitting'} details={[{ label: '품목 분류', value: currentCategory?.label ?? '확인 불가' }, { label: '결과', value: recipe?.label ?? '미선택' }, { label: '필요 Echo', value: recipe?.requiredEchoes.join(', ') || '표시 없음' }, { label: '비용', value: money(recipe?.cost) }]} onCancel={() => setConfirming(false)} onConfirm={() => categoryId && recipe && void town.submit({ categoryCandidateId: categoryId, recipeCandidateId: recipe.id }).catch(() => setConfirming(false))} />} />;
}
