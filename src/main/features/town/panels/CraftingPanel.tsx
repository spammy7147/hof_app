...
function CraftingHistoryScreen({ rows, onBack }: { rows: TownRowResponse[]; onBack: () => void }) { return <View style={styles.container}><ActionButton label="제작 화면으로 돌아가기" disabled={false} onPress={onBack} /><TownItemList rows={rows} selectionMode="none" header={<View style={styles.historyHeader}><Text accessibilityRole="header" style={styles.title}>Hall of Pain 기록</Text><Text style={styles.hint}>최근 제련 결과 {rows.length.toLocaleString()}건을 확인합니다.</Text></View>} emptyMessage="저장된 Hall of Pain 기록이 없습니다." /></View>; }
