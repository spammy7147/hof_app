  ...
  return <View style={styles.container}>
    <View style={styles.listArea}>
      <TownItemList rows={rows} selectedIds={selected} selectionMode="multiple" onSelectionChange={(ids) => { setCart(Object.fromEntries(ids.map((id) => [id, cart[id] ?? 1]))); setInvalidQuantities((current) => Object.fromEntries(ids.map((id) => [id, current[id] ?? false]))); }}
        stickyHeader
        header={<View style={styles.searchHeader}>{data.stale ? <Text style={styles.warning}>상점 검증이 지연되어 마지막 확인 목록을 표시합니다.</Text> : null}<TextInput accessibilityLabel="상점 품목 검색" onChangeText={setQuery} placeholder="이름·유형·설명·가격 검색" placeholderTextColor={theme.colors.textMuted} style={styles.input} value={query} /></View>}
...
const styles = StyleSheet.create({
  container: { flex: 1, gap: theme.spacing.sm }, listArea: { flex: 1, minHeight: 0 }, section: { gap: theme.spacing.sm, paddingVertical: theme.spacing.sm }, searchHeader: { backgroundColor: theme.colors.background, gap: theme.spacing.xs, paddingBottom: theme.spacing.sm }, quantityField: { gap: 2 }, inputLabel: { color: theme.colors.text, fontSize: 12, fontWeight: '800', lineHeight: 16 }, muted: { color: theme.colors.textMuted }, warning: { color: theme.colors.accentAmber, fontWeight: '800' }, error: { color: theme.colors.danger }, total: { color: theme.colors.accentGreen, fontSize: 15, fontWeight: '900' },
  ...
  quantityInput: { minHeight: 40, paddingVertical: 4 },
  invalidInput: { borderColor: theme.colors.danger },
  button: { alignItems: 'center', backgroundColor: theme.colors.accentGreen, borderRadius: theme.radius.md, justifyContent: 'center', minHeight: 44, paddingHorizontal: theme.spacing.md, paddingVertical: theme.spacing.sm }, buttonText: { color: theme.colors.background, fontWeight: '900' }, disabled: { opacity: 0.45 },
  purchaseActionBar: { backgroundColor: theme.colors.surface, borderColor: theme.colors.borderStrong, borderRadius: theme.radius.md, borderWidth: 1, gap: theme.spacing.xs, padding: theme.spacing.sm },
  sellActionBar: { backgroundColor: theme.colors.surface, borderColor: theme.colors.borderStrong, borderRadius: theme.radius.md, borderWidth: 1, gap: theme.spacing.xs, padding: theme.spacing.sm },
