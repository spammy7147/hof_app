...
export function ExchangePanel({ api, mode, resolveCaptcha }: Props) {
  ...
  const [suppressedResultKey, setSuppressedResultKey] = useState<string | null>(null);
  const staged = useRef<Loaded | null>(null);
  ...
  const submitAction = useCallback(async (mutation: Mutation) => {
    ...
    if (activeFeatureKey.current === featureKey) staged.current = { key: featureKey, value: next };
    return next.result ?? informational('시설 정보를 갱신했습니다.');
  }, [api, featureKey, mode]);
  const town = useTownFeature<Loaded, Mutation>({ load, submitAction, resolveCaptcha, featureKey, describeError: toUserFacingErrorMessage });
  ...
  const interactionBusy = town.status === 'loading' || town.status === 'submitting';
  const rows: TownRowResponse[] = [
    ...data.rows.map((row) => toTownRow(`trade:${row.id}`, row, '교환 품목', interactionBusy)),
    ...data.annActions.flatMap((action) => action.rows.map((row) => toTownRow(`ann:${action.type}:${row.id}`, row, action.label, interactionBusy))),
  ...
  const footer = <View style={styles.section}>
    ...
    {displayedResult ? <TownActionResult result={displayedResult} onRefresh={interactionBusy ? undefined : () => {
      const refreshKey = featureKey;
      setResponse(null); setActionResult(null); setSuppressedResultKey(refreshKey);
      void town.reload().then(() => { if (activeFeatureKey.current === refreshKey) setSuppressedResultKey(null); }).catch(() => undefined);
    }} /> : null}
