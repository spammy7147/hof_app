...
export function TownActionResult({ result, onRefresh, successNoticeTitle }: TownActionResultProps) {
...
function noticeAccessibilityLabel(status: TownResultStatus, successTitle?: string): string {
  ...
  if (status === 'INFORMATIONAL') return '작업 안내 알림';
  return '결과 확인 필요 알림';
}
...
function statusMark(status: TownResultStatus): string {
  ...
  if (status === 'INFORMATIONAL') return 'i';
  return '?';
}
