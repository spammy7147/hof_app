  ...
  return <NestableScrollContainer contentContainerStyle={styles.container}>
    ...
    {cycles.map((cycle) => <View key={cycle.id} style={styles.cycle}>
      ...
      {cycle.events.map((event) => <View key={event.id} style={styles.event}>
        <Text style={styles.sequence}>{event.sequence + 1}</Text><View style={styles.eventCopy}><Text style={styles.eventTitle}>{event.type ? AUTOMATION_TYPE_METADATA[event.type].label : '시스템'} · {kindLabel(event.kind)}</Text><Text style={styles.message}>{event.message}</Text>{event.nextRunAt ? <Text style={styles.next}>다음 확인 {new Date(event.nextRunAt).toLocaleString('ko-KR')}</Text> : null}</View>
      </View>)}
    </View>)}
