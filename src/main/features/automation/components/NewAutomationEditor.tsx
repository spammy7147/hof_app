...
export function NewAutomationEditor({ entry, saving, mutationMessage, onBack, onLoadBattleMaps, onLoadRaidTargets, partyPresetCatalog, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    const request = entry.type === 'UNION'
  ...
  const activePreset = activePresetKey === 'FISHING'
    ? fishingPreset
    : selected.find(({ key }) => key === activePresetKey)?.preset ?? null;
  const activePresetName = activePresetKey === 'FISHING'
    ? '낚시 전투'
    : selected.find(({ key }) => key === activePresetKey)?.name ?? '';
  ...
  return <NestableScrollContainer contentContainerStyle={styles.container}>
    ...
    {entry.type === 'FISHING' ? <View style={styles.presetBox}>
      <Text style={styles.section}>전투 프리셋</Text>
      <PresetTrigger name="낚시" value={fishingPreset} presets={presets} onPress={() => setActivePresetKey('FISHING')} />
    </View> : <>
    ...
    <BattleMapPresetPickerModal
      ...
      onSelect={(presetId) => {
        ...
        if (activePresetKey === 'FISHING') setFishingPreset(preset);
        else if (activePresetKey != null) setTargetPreset(activePresetKey, preset);
